(function($) {
    ...
    $(document).on('add.cards change.cards', function(event) {
        if(typeof $.fn.masonry !== 'undefined') {
            $(event.target).outerFind('.mbr-portfolio').each(function() {
                var $msnr = $(this).find('.mbr-portfolio-row').masonry({
                    itemSelector: '.mbr-portfolio-item',
    ...
    function fitLightbox() {
        var $lightbox = $('.mbr-portfolio .modal');
        if(!$lightbox.length) {
            return;
        }
